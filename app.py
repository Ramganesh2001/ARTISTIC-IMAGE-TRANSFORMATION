...
def cartoonize_2(image, kernel_size=15, edge_darkening=0.4):
    # Convert the image to grayscale
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)

    # Apply bilateral filter for smoothing while preserving edges
    smoothed = cv2.bilateralFilter(gray, d=kernel_size, sigmaColor=75, sigmaSpace=75)

    # Create an edge mask using adaptive thresholding
    edge_mask = cv2.adaptiveThreshold(smoothed, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, blockSize=kernel_size, C=edge_darkening)

    # Convert the image to a watercolor-like painting by combining the edge mask and the original image
    watercolor = cv2.bitwise_and(image, image, mask=edge_mask)
...
def cartoonize_3(img):
    ...
    # Convert the one-channel grayscale background to a three-channel image
...
def cartoonize_6(img):
    ...
    dst = cv2.detailEnhance(img, sigma_s=30, sigma_r=0.50)
